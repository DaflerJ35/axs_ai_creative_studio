...
Production domain: **[axs-ai-creative-studios.com](https://axs-ai-creative-studios.com)**
...
docker build --build-arg HF_TOKEN=hf_xxxxx -t yourname/axs-flux:latest .
docker push yourname/axs-flux:latest
```

### Create the serverless endpoint

1. Go to **RunPod → Serverless → New Endpoint**
2. Container: `yourname/axs-flux:latest`
...
### Vercel (recommended for `axs-ai-creative-studios.com`)

1. Push this repo to GitHub
2. In Vercel: **New Project → Import → Framework: Vite**
3. Build command: `npm run build`
4. Output: `dist`
5. Go to **Settings → Domains → Add `axs-ai-creative-studios.com`**
...
- Characters, gallery, and settings live in **IndexedDB** (`axs-vault-v1`).
- API keys mirror into `localStorage` under `axs.apiKey`, `axs.endpointId`, `axs.videoEndpointId` (read by the RunPod client).
