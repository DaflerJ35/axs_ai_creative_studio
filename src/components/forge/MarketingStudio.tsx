...
export const MarketingStudio = () => {
  const { settings, addToGallery, brandVoice } = useAxsStore();
  ...
  const handleGenerateCampaign = async () => {
    ...
    try {
      for (let i = 0; i < jobs.length; i++) {
        ...
        const prompt = `${buildCampaignPrompt(businessMode, productName, productDesc, adStyle, hook)} Brand voice direction: ${brandVoice.tone}; cadence: ${brandVoice.cadence}; avoid: ${brandVoice.forbiddenPhrases.join(", ") || "generic filler"}.`;
        ...
        try {
          ...
          if (out.status === "success" && out.images?.length) {
            ...
            const url = imageOutputToUrl(img.image, img.mime);
  ...
  const handleCopyHook = async (hook: string) => {
    ...
    if (copied) toast.success("Hook copied to clipboard");
              ...
              <GlassCard className="p-8">
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <Select value={adStyle} onValueChange={(value) => value && setAdStyle(value)}>
