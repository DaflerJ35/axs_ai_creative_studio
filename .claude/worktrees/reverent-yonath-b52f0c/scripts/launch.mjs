...
const DEFAULT_ARGS = "-s --windows-standalone-build --enable-cors-header";
...
async function main() {
  console.log(`
${c.violet}${c.bold}  NYXforge${c.reset} ${c.dim}— unified launcher${c.reset}
...
`);
  ...
  comfyProc = spawnProc(
    COMFY,
    pythonExe,
    [mainPy, "--listen", "127.0.0.1", "--port", "8188", ...extraArgs],
    { cwd: comfyDir }
  );
  ...
  viteProc = spawnProc(VITE, npx, ["vite", "--port=3000", "--host=0.0.0.0"]);
