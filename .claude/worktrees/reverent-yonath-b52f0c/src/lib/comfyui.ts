...
const getBase = (): string =>
  (localStorage.getItem("momentum.comfyuiUrl") || "http://127.0.0.1:8188").replace(/\/$/, "");
const getModel = (): string => localStorage.getItem("momentum.comfyuiModel") || "";
const getLoraBase = (): string => localStorage.getItem("momentum.comfyuiLoraBase") || "";
...
export function detectModelType(filename: string): ModelType {
  const n = filename.toLowerCase();
  if ((n.includes("wan") && (n.includes("t2v") || n.includes("i2v"))) || n.includes("_t2v_")) return "video";
  if (n.includes("flux")) return "flux";
  if (n.match(/v2[_-]1/) || (n.includes("768") && n.includes("pruned"))) return "sd21";
  if (n.includes("lora") || n.match(/-\d{6}\.safetensors$/) || n.match(/-\d{6}-/)) return "lora";
  return "sdxl";
}
...
function saveNodes(fromLatent: string) {
  return {
    ...
    "9": { class_type: "SaveImage", inputs: { filename_prefix: "momentum", images: ["8", 0] } },
  };
}
...
function buildWorkflow(p: ComfyParams): Record<string, unknown> {
  ...
  switch (type) {
    ...
    case "video": throw new Error(`"${model}" is a video model — use the Videos tab.`);
    case "lora":  throw new Error(`"${model}" is a LoRA file — select a full checkpoint as the base model, then pick this LoRA from the LoRA slot below.`);
    default: {
      if (isLightning(model)) return buildLightningWorkflow(p, model);
...
async function pollHistory(promptId: string, onProgress?: (i: number) => void): Promise<ComfyOutputs> {
  for (let i = 0; i < 180; i++) {
    ...
    if (err) throw new Error(`ComfyUI: ${(err[1] as any)?.exception_message ?? JSON.stringify(err[1])}`);
...
export async function comfyGenerate(params: ComfyParams, onProgress?: (i: number) => void): Promise<ImageJobOutput> {
  ...
  const images: Array<{ image: string; seed: number; index: number }> = [];
  let index = 0;
  for (const nodeOut of Object.values(outputs)) {
    if (nodeOut.images) {
      for (const img of nodeOut.images) {
        const dataUrl = await imageToBase64(img.filename, img.subfolder, img.type);
        images.push({ image: dataUrl.replace(/^data:[^;]+;base64,/, ""), seed, index: index++ });
      }
    }
  }
...
export async function testComfyUIConnection(url?: string): Promise<{ ok: boolean; models?: string[]; loras?: string[]; gpuName?: string; error?: string }> {
  ...
  try {
    const [statsRes, modelsRes, lorasRes] = await Promise.all([
      fetch(`${base}/system_stats`, { signal: AbortSignal.timeout(5000) }),
      fetch(`${base}/object_info/CheckpointLoaderSimple`, { signal: AbortSignal.timeout(5000) }),
      fetch(`${base}/object_info/LoraLoader`, { signal: AbortSignal.timeout(5000) }),
    ]);
