...
import {
  ...
  Play, Lightbulb, Download, Star, ArrowRight, Image, List,
} from "lucide-react";
...
export const ScriptForge = () => {
  const { setDraftPrompt, setActiveTab } = useNyxStore();
  ...
  const handleGenerate = async () => {
    ...
    const generated = [
      generateScript(framework, product.trim(), avatar.trim(), angle.trim(), platform),
      generateScript(framework, product.trim(), avatar.trim(), angle.trim(), platform),
    ];
  ...
  return (
    <div className="space-y-10">
      {/* ── Hero ─────────────────────────────────────────────────────────────── */}
      <div>
        <div className="text-xs uppercase tracking-[0.3em] text-white/40 mb-2">Script Forge</div>
        ...
        <p className="text-white/50 mt-4 max-w-2xl text-lg">
          Five battle-tested ad frameworks used by 8-figure agencies.
          Every section timed, every line tested — with the performance tips they don't put in the course.
        </p>
        <div className="flex flex-wrap gap-3 mt-5">
          {["higgsfield.ai has zero of this", "used by $50M/yr ad buyers", "8 proven frameworks"].map((tag) => (
            <span key={tag} className="text-xs px-3 py-1 rounded-full border border-white/10 text-white/40 bg-white/[0.03]">
              {tag}
            </span>
          ))}
        </div>
      ...
      {/* ── Framework Picker ─────────────────────────────────────────────────── */}
      <div>
        <div className="text-xs uppercase tracking-[0.3em] text-white/40 mb-4">Ad Framework</div>
            ...
            return (
              ...
              >
                {isActive && (
                  <motion.div
                    layoutId="fw-glow"
                    className={`absolute inset-0 rounded-2xl bg-gradient-to-br ${f.color} opacity-10`}
                  />
                )}
                <div className="relative z-10">
                  <div className={`text-xl font-black mb-1 bg-gradient-to-r ${f.color} bg-clip-text text-transparent`}>
                    {f.name}
                  </div>
                  <div className="text-[11px] text-white/50 leading-tight">{f.full}</div>
                  <div className="flex gap-0.5 mt-2">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star
                        key={i}
                        className={`w-2.5 h-2.5 ${i < f.conversionRating ? "text-amber-400 fill-amber-400" : "text-white/10"}`}
                      />
                    ))}
                  </div>
                </div>
          ...
          <GlassCard intensity="light" className="p-5">
            <div className="flex items-start gap-4 flex-wrap">
              <div className="flex-1 min-w-0">
                <div className={`text-sm font-bold mb-1 bg-gradient-to-r ${activeFramework.color} bg-clip-text text-transparent`}>
                  {activeFramework.full}
                </div>
                <p className="text-white/60 text-sm">{activeFramework.description}</p>
              </div>
              <div className="flex flex-wrap gap-2">
                {activeFramework.bestFor.map((tag) => (
                  <span key={tag} className="text-[11px] px-2.5 py-1 rounded-full bg-white/[0.06] border border-white/10 text-white/60">
                    {tag}
                  </span>
                ))}
              </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <GlassCard className="p-7 space-y-5">
          <div className="text-xs uppercase tracking-[0.3em] text-white/40">Campaign Brief</div>
          <div>
            <Label>Product / Brand</Label>
            <Input
              value={product}
              onChange={(e) => setProduct(e.target.value)}
              placeholder="Aurora Silk Serum"
              className="mt-1"
            />
          </div>
          <div>
            ...
            <Input
              ...
              placeholder="Women 28–45, skincare-obsessed, $80k+ HHI"
              className="mt-1"
            />
          </div>
          <div>
            <Label>Core Angle / Key Benefit</Label>
            <Textarea
              rows={2}
              value={angle}
              onChange={(e) => setAngle(e.target.value)}
              placeholder="The only serum that actually penetrates the dermis layer — most don't."
              className="mt-1"
            />
            <p className="text-xs text-white/30 mt-1.5">
              This is your unique mechanism. The more specific, the better the output.
            </p>
          </div>
        </GlassCard>

        <GlassCard className="p-7 space-y-5">
          <div className="text-xs uppercase tracking-[0.3em] text-white/40">Platform & Duration</div>
            ...
            {PLATFORMS.map((p) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-white/40">max {p.maxSeconds}s</span>
                  {platform === p.id && <CheckCircle className="w-4 h-4 text-violet-400" />}
                </div>
              </button>
            ))}
          </div>
      ...
      <Button
        ...
        disabled={loading}
        className={`
          w-full h-16 text-lg font-black rounded-2xl
          bg-gradient-to-r ${activeFramework.color} text-black
          hover:brightness-110 active:scale-[0.99]
          ${activeFramework.glow}
          disabled:opacity-50
        `}
      >
        {loading ? (
          ...
          <div className="flex items-center gap-3">
            ...
            Write {activeFramework.name} Scripts
      ...
      <AnimatePresence>
        {scripts.length > 0 && currentScript && (
          <motion.div
            ...
            className="space-y-6"
          >
            {/* Variation tabs + export */}
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div className="flex gap-2">
                {scripts.map((_, i) => (
                  <button
                    key={i}
                    onClick={() => { setActiveScript(i); setExpandedSection(0); }}
                    className={`px-4 py-2 rounded-xl text-sm font-bold border transition-all ${
                      activeScript === i
                        ? `bg-gradient-to-r ${activeFramework.color} text-black border-transparent`
                ...
                <button
                  onClick={handleGenerate}
                  className="px-3 py-2 rounded-xl text-sm border border-white/10 text-white/40 hover:text-white hover:border-white/20 transition-all flex items-center gap-1.5"
                >
                ...
                <button
                  onClick={() => {
                    currentScript.sections.forEach((s, i) => {
                      setTimeout(() => {
                        setDraftPrompt(s.visualPrompt + (character ? ` featuring ${character.name}` : ""));
                      }, i * 200);
                    });
                    setActiveTab("images");
                    toast.success(`${currentScript.sections.length} prompts queued in Image Forge`);
                  }}
                  className={`px-3 py-2 rounded-xl text-xs font-bold bg-gradient-to-r ${activeFramework.color} text-black hover:brightness-110 transition-all flex items-center gap-1.5`}
                >
                  <Image className="w-3.5 h-3.5" /> Generate All Images
                </button>
                <button
                  onClick={() => handleExportScript(currentScript)}
                  className="px-3 py-2 rounded-xl text-xs border border-white/10 text-white/50 hover:text-white hover:border-white/20 transition-all flex items-center gap-1.5"
                >
                  <Download className="w-3.5 h-3.5" /> Export .txt
                </button>
                <button
                  onClick={() => {
                    const full = currentScript.sections.map((s) => `[${s.title}]\n${s.lines}`).join("\n\n");
                    handleCopy(full, "Full script");
                  }}
                  className="px-3 py-2 rounded-xl text-xs border border-white/10 text-white/50 hover:text-white hover:border-white/20 transition-all flex items-center gap-1.5"
                >
            ...
            {/* Script metadata + view tabs */}
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div className="flex items-center gap-4 text-sm">
                <div className="flex items-center gap-1.5 text-white/50">
                  <Clock className="w-3.5 h-3.5" />
                  {currentScript.totalDuration}
                </div>
                <div className="flex items-center gap-1.5 text-white/50">
                  <BookOpen className="w-3.5 h-3.5" />
                  {currentScript.sections.length} sections
                </div>
                <div className={`text-xs px-2.5 py-1 rounded-full bg-gradient-to-r ${activeFramework.color} bg-opacity-20 text-white font-medium`}>
                  {currentScript.frameworkName}
                </div>
              </div>
                ...
                {([
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                      outputTab === id ? "bg-white/10 text-white" : "text-white/40 hover:text-white/70"
                    }`}
                  >
            ...
            {outputTab === "shotlist" && (
              <GlassCard className="overflow-hidden">
                <div className="p-5 border-b border-white/10">
                  <div className="text-xs uppercase tracking-widest text-white/40">Auto Shot List — {currentScript.frameworkName}</div>
                </div>
                <div className="divide-y divide-white/[0.06]">
                  {buildShotList(currentScript).map((shot) => (
                    <div key={shot.shot} className="flex items-start gap-4 p-4 hover:bg-white/[0.02] transition-colors">
                      <div className={`w-8 h-8 rounded-lg flex-shrink-0 flex items-center justify-center text-xs font-black bg-gradient-to-br ${activeFramework.color} text-black`}>
                        {shot.shot}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="font-semibold text-sm">{shot.title}</div>
                        <div className="text-xs text-white/40 mt-0.5">{shot.promptPreview}</div>
                      </div>
                      <div className="text-right flex-shrink-0 space-y-1">
                        <div className="text-xs text-white/50 font-mono">{shot.duration}</div>
                        <div className="text-[10px] text-white/30">{shot.camera}</div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {outputTab === "storyboard" && (
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {currentScript.sections.map((section, i) => (
                  <div key={i} className="overflow-hidden group rounded-2xl border border-white/10 bg-white/[0.04]">
                    <div className="aspect-video bg-gradient-to-br from-white/[0.03] to-white/[0.08] flex items-center justify-center relative">
                      <div className="text-center p-4">
                        <div className="text-xs text-white/30 mb-2">Shot {i + 1} · {section.timeCode}</div>
                        <div className="text-[11px] text-white/60 leading-snug">{section.visualPrompt.slice(0, 100)}…</div>
                      </div>
                      <button
                        onClick={() => handleForgeVisual(section)}
                        className={`absolute bottom-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity px-3 py-1.5 rounded-lg text-[11px] font-bold bg-gradient-to-r ${activeFramework.color} text-black flex items-center gap-1`}
                      >
                ...
                >
                  <GlassCard
                    className={`overflow-hidden transition-all duration-300 ${
                      expandedSection === i ? "border-violet-500/30" : ""
                    }`}
                  >
                    <button
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-black bg-gradient-to-br ${activeFramework.color} text-black`}>
                          {i + 1}
                        </div>
                        <div>
                      ...
                      <ChevronRight
                        className={`w-4 h-4 text-white/30 transition-transform ${expandedSection === i ? "rotate-90" : ""}`}
                      />
                    </button>
                      ...
                      {expandedSection === i && (
                        ...
                        >
                          <div className="px-5 pb-5 space-y-5">
                            {/* Script lines */}
                            <div className="relative p-5 rounded-2xl bg-white/[0.04] border border-white/[0.08]">
                              <div className="text-sm leading-relaxed text-white/90 font-medium">
                                {section.lines}
                              </div>
                              <div className="flex gap-2 mt-4">
                                <button
                                  onClick={() => handleCopy(section.lines, section.title)}
                                  className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg bg-white/[0.07] hover:bg-white/[0.12] text-white/60 hover:text-white transition-all"
                                >
                                  <Copy className="w-3 h-3" /> Copy lines
                                </button>
                                <button
                                  onClick={() => handleForgeVisual(section)}
                                  className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg bg-gradient-to-r ${activeFramework.color} text-black font-bold transition-all hover:brightness-110`}
                                >
                                  <Zap className="w-3 h-3" /> Forge Visual
                                </button>
                              </div>
                            </div>
                            ...
                            <div>
                              <div className="flex items-center gap-2 mb-3">
                                ...
                                <span className="text-xs uppercase tracking-wider text-white/40">Performance Tips</span>
                              </div>
                            ...
                            <div className="p-3 rounded-xl bg-white/[0.03] border border-white/[0.06]">
                              <div className="text-[11px] uppercase tracking-wider text-white/30 mb-1.5">Visual Prompt for this section</div>
                              <div className="text-xs text-white/50">{section.visualPrompt}</div>
                            </div>
                          </div>
                        </motion.div>
                      )}
            ...
            <div className="grid md:grid-cols-2 gap-5">
              <GlassCard className="p-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <div className="text-xs uppercase tracking-wider text-white/40">Hook Variations</div>
                </div>
                <div className="space-y-3">
                  {currentScript.hookVariations.map((h, i) => (
                    <div key={i} className="group flex items-start gap-3 p-3 rounded-xl bg-white/[0.03] border border-white/[0.05] hover:border-white/10 transition-all">
                      <Play className="w-3.5 h-3.5 text-white/30 mt-0.5 flex-shrink-0" />
              ...
              <GlassCard className="p-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <div className="text-xs uppercase tracking-wider text-white/40">CTA Variations</div>
                </div>
                <div className="space-y-3">
                  {currentScript.ctaVariations.map((c, i) => (
                    <div key={i} className="group flex items-start gap-3 p-3 rounded-xl bg-white/[0.03] border border-white/[0.05] hover:border-white/10 transition-all">
                      <ArrowRight className="w-3.5 h-3.5 text-white/30 mt-0.5 flex-shrink-0" />
                      <span className="text-xs text-white/70 flex-1">{c}</span>
                      <button
                        onClick={() => handleCopy(c, "CTA")}
                        className="opacity-0 group-hover:opacity-100 transition-opacity"
                      >
                        <Copy className="w-3 h-3 text-white/40 hover:text-white/70" />
                      </button>
